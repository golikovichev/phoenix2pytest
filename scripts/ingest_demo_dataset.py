...
def validate_dataset(traces: list[dict[str, Any]]) -> None:
    ...
    are recognised, and synthetic entries have a non-null llm_output
    while real entries have llm_output=None.
    """
    ...
    for index, trace in enumerate(traces):
        ...
        if trace["source"] == "synthetic" and not trace["llm_output"]:
            raise ValueError(f"trace {trace_id!r} is synthetic but has empty llm_output")
...
def split_by_source(traces: list[dict[str, Any]]) -> tuple[list[dict], list[dict]]:
    """Return (real_traces, synthetic_traces)."""
    real = [t for t in traces if t["source"] == "real"]
    synthetic = [t for t in traces if t["source"] == "synthetic"]
